...
def load_articles(sector=None):
    query = """
    SELECT title, sector, sentiment, sentiment_score, published_at, url , source
    FROM articles
...
# Load data
sectors = ['Select','AI', 'EV', 'Green Energy', 'Fashion Tech', 'Healthcare', 'FinTech', 'EdTech', 'Cybersecurity', 
...
if selected_sector == 'Select':
    st.warning("Please select a sector to view articles.")
else:
...
if df.empty:
    ...
    for _, row in top_positive.iterrows():
        st.markdown(f"**[{row['sector']}]** {row['title']} ({row['sentiment_score']:.2f}) — *{row['published_at'].strftime('%Y-%m-%d')}* {row['url']}")
    ...
    pdf_buffer = create_pdf_report(filtered_df,'/tmp/sentiment_pie_chart.png', '/tmp/sentiment_time_series.png')
